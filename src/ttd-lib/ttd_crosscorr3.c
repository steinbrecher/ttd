  ...
  if ((other_rb1_count > 0) && (other_rb2_count > 0)) {
    for (m = 0; m < other_rb2_count; m++) {
      for (n = 0; n < other_rb1_count; n++) {
        ...
        // Column Delta
        delta_bins1 = ((int64_t) ccorr->center_bin +
                       int64_rounded_divide(times[1] - times[0], (int64_t) ccorr->bin_time));

        // Row Delta
        delta_bins2 = ((int64_t) ccorr->center_bin +
                       int64_rounded_divide(times[2] - times[0], (int64_t) ccorr->bin_time));
